...
from PritiMusic.utils.formatters import get_readable_time, time_to_seconds

# Config Imports
from config import BANNED_USERS, OWNER_ID, START_IMG_URL, CMBOT, EFFECT_ID, SUPPORT_CHAT, OWNER_USERNAME
...
# =====================================================================
# 🔥 PREMIUM EMOJIS & DYNAMIC BUTTON LOGIC
# =====================================================================
...
def make_start_panel(bot_username, owner_url, 
    ...
    # 1. Add to Group
    if txt_add != "HIDDEN":
        buttons.append([create_btn(text=txt_add, url=f"https://t.me/{bot_username}?startgroup=true", style=style_map[1])])

    # 2. Help Button
    if txt_help != "HIDDEN":
        buttons.append([create_btn(text=txt_help, callback_data="settings_back_helper", style=style_map[2])])

    # 3. Support & Channel (Row)
    row_support = []
    if txt_support != "HIDDEN":
        row_support.append(create_btn(text=txt_support, url=support_chat, style=style_map[3]))
    if txt_channel != "HIDDEN":
        row_support.append(create_btn(text=txt_channel, url=support_channel, style=style_map[3]))
    if row_support:
        buttons.append(row_support)

    # 4. Owner Button
    if txt_owner != "HIDDEN":
        buttons.append([create_btn(text=txt_owner, url=owner_url, style=style_map[4])])
    ...
    if custom_btn and custom_btn.get("text"):
        ...
        c_btn = create_btn(text=custom_btn["text"], url=btn_url, style=style_map[5])
...
def make_gp_panel(bot_username, txt_add, txt_support, support_chat):
    ...
    buttons = [
        [
            create_btn(text=txt_add, url=f"https://t.me/{bot_username}?startgroup=true", style=style_map[1]),
            create_btn(text=txt_support, url=support_chat, style=style_map[2]),
        ]
    ]
    return InlineKeyboardMarkup(buttons)
...
# ⚡ Background Delete Helper to boost speed
...
async def start_pm(client, message: Message, _):
    ...
    if custom_sticker:
        try: loading = await message.reply_sticker(custom_sticker)
        except: pass
    elif custom_animation:
        try: loading = await message.reply_animation(custom_animation)
        except: pass
    else:
        try: loading = await message.reply_text("<b>ʟᴏᴀᴅɪɴɢ...</b>", parse_mode=ParseMode.HTML)
        except: pass

    # Fetch All Data Parallelly
    (
        C_BOT_OWNER_ID, raw_support, raw_channel, txt_add, txt_support,
        txt_channel, txt_owner, txt_help, raw_custom_btn, btn_pos,
        raw_video, raw_img, raw_caption, raw_reaction, raw_effect
    ) = await asyncio.gather(
        get_owner_id_from_db(bot_id), get_cloned_support_chat(bot_id),
        get_cloned_support_channel(bot_id), get_custom_btn_text(bot_id, "add", _["S_B_3"]),
        get_custom_btn_text(bot_id, "support", _["S_B_9"]), get_custom_btn_text(bot_id, "channel", _["S_B_6"]),
        get_custom_btn_text(bot_id, "owner", _["C_B_2"]), get_custom_btn_text(bot_id, "help", _["S_B_4"]),
        get_start_button(bot_id), get_start_btn_pos(bot_id), get_start_video(bot_id),
        get_start_image(bot_id), get_start_caption(bot_id), get_start_reaction(bot_id),
    ...
    # 1. RANDOM REACTION
    reaction_emoji = random.choice(raw_reaction.split("|||")) if raw_reaction else random.choice(["🔥", "❤️", "🥰", "😍", "👍", "⚡", "🎉"])
    try: asyncio.create_task(message.react(reaction_emoji))
    except: pass

    # ⚡ 2. Delete loading in background instantly
    if loading: asyncio.create_task(safe_delete(loading))

    # Inline Arguments Handle
    if len(message.text.split()) > 1:
        arg = message.text.split(None, 1)[1]
        style_map = get_style_map()
        if arg.startswith("help"):
            keyboard = InlineKeyboardMarkup([[create_btn(_["S_B_9"], url=C_SUPPORT_CHAT, style=style_map[1])]])
            return await message.reply_photo(photo=get_random_start_image(), caption=_["help_1"].format(C_SUPPORT_CHAT), reply_markup=keyboard, has_spoiler=True)
        ...
        if arg.startswith("inf"):
            ...
            try:
                ...
                key = InlineKeyboardMarkup([[create_btn(_["S_B_8"], url=result["link"], style=style_map[1]), create_btn(_["S_B_9"], url=C_SUPPORT_CHAT, style=style_map[2])]])
                asyncio.create_task(safe_delete(m))
    ...
    markup = make_start_panel(a.username, OWNER_URL, txt_add, txt_support, txt_channel, txt_owner, txt_help, C_SUPPORT_CHAT, C_SUPPORT_CHANNEL, custom_button_data, btn_pos)
    ...
    bot_mention = a.mention
    
    if custom_caption:
        try:
            caption = custom_caption.format(name=user_mention, firstname=message.from_user.first_name, botname=bot_mention, username=a.username)
        except:
...
async def start_gp(client, message: Message, _):
    ...
    raw_support, txt_add, txt_support, raw_video, raw_img = await asyncio.gather(
        get_cloned_support_chat(a.id), get_custom_btn_text(a.id, "add", _["S_B_1"]),
        get_custom_btn_text(a.id, "support", _["S_B_2"]), get_start_video(bot_id), get_start_image(bot_id)
    )
    ...
    if start_video:
        try: return await message.reply_video(start_video, caption=caption, reply_markup=markup, has_spoiler=True)
        except: pass
...
async def home_back_handler(client, CallbackQuery, _):
    ...
    (
        C_BOT_OWNER_ID, raw_support, raw_channel, txt_add, txt_support,
        txt_channel, txt_owner, txt_help, raw_custom_btn, btn_pos,
        raw_video, raw_img, raw_caption, raw_effect
    ) = await asyncio.gather(
        get_owner_id_from_db(bot_id), get_cloned_support_chat(bot_id),
        get_cloned_support_channel(bot_id), get_custom_btn_text(bot_id, "add", _["S_B_3"]),
        get_custom_btn_text(bot_id, "support", _["S_B_9"]), get_custom_btn_text(bot_id, "channel", _["S_B_6"]),
        get_custom_btn_text(bot_id, "owner", _["C_B_2"]), get_custom_btn_text(bot_id, "help", _["S_B_4"]),
        get_start_button(bot_id), get_start_btn_pos(bot_id), get_start_video(bot_id),
        get_start_image(bot_id), get_start_caption(bot_id), get_start_effect(bot_id),
    )
    ...
    custom_button_data = None
    if raw_custom_btn:
        if isinstance(raw_custom_btn, dict): custom_button_data = raw_custom_btn
        elif isinstance(raw_custom_btn, str):
            chosen_str = random.choice(raw_custom_btn.split("|||"))
            if "-" in chosen_str:
                txt, url = chosen_str.split("-", 1)
    ...
    if custom_caption:
        try: caption = custom_caption.format(name=user_mention, firstname=CallbackQuery.from_user.first_name, botname=bot_mention, username=a.username)
        except: caption = custom_caption
    else:
        formatted_text = f"Hey {user_mention} 👋\n\n⦿ THIS IS {bot_mention} !\n\n➻ A FAST & POWERFUL TELEGRAM MUSIC PLAYER BOT.\n\n──────────────────\n✦ POWERED BY » {bot_mention}"
        caption = f"<blockquote expandable>{formatted_text}</blockquote>"
...
async def transfer_owner(client, message):
    ...
    if message.reply_to_message:
        ...
        try: new_owner = await client.get_users(message.command[1])
        except: return await message.reply_text("❌ User not found! Check Username or ID.")
    else: return await message.reply_text("❌ **Usage:**\nReply to a user or type `/transfer @username`.")

    if new_owner.is_bot: return await message.reply_text("❌ You cannot make a bot the owner.")
    if new_owner.id == user.id: return await message.reply_text("❌ You are already the owner.")
...
async def set_start_reaction_cmd(client, message):
    bot_id = (await client.get_me()).id
    if len(message.command) < 2: return await message.reply_text("❌ **Usage:** `/setstartreaction 🔥`\nYou can add multiple.")
    await add_start_content(bot_id, "start_reaction", message.command[1])
    await message.reply_text(f"✅ Start Reaction Added: {message.command[1]}")
...
async def set_start_effect_cmd(client, message):
    bot_id = (await client.get_me()).id
    if len(message.command) < 2: return await message.reply_text("❌ **Usage:** `/setstarteffect 🔥` or ID\n\nSupported: 🔥, 👍, 👎, ❤️, 🎉, 💩")
    
    EFFECT_MAP = {
        "🔥": "5104841245755180586", "👍": "5107584321108051014", "👎": "5104858069142078462",
        "❤️": "5044134455711629726", "🎉": "5046509860389126442", "💩": "5046589136895476101"
    }
...
async def set_start_image_cmd(client, message):
    ...
    else: await message.reply_text("Reply to a photo.")
...
async def set_start_video_cmd(client, message):
    ...
    else: await message.reply_text("Reply to a video.")
...
async def set_start_sticker_cmd(client, message):
    ...
    else: await message.reply_text("Reply to a sticker.")
...
async def set_start_animation_cmd(client, message):
    ...
    else: await message.reply_text("Reply to a GIF.")
...
async def set_start_caption_cmd(client, message):
    ...
    else: await message.reply_text("Reply to a text to add as Caption.")
...
async def set_btn_pos_cmd(client, message):
    bot_id = (await client.get_me()).id
    if len(message.command) < 2: return await message.reply_text("Usage: `/setbtnpos [UP/DOWN/MID]`")
    ...
    else: await message.reply_text("❌ Invalid! Use: UP, DOWN, MID")
