...
# API Provider Sequence (Bot sabse pehle Inflex, fir ShrutiBots try karega)
API_PROVIDERS = [
    ...
    async with aiohttp.ClientSession() as session:
        for provider in API_PROVIDERS:
            ...
            try:
                ...
                # FIX: Timeout ko 60s se 10s kar diya hai taaki delay na aaye
...
# FIX: Yahan JioSaavn ko priority me add kar diya gaya hai
async def download_song(link: str, title: str = None) -> str:
    ...
    if title:
        jio_result = await jiosaavn_fallback_download(title)
        if jio_result: return jio_result
...
class YouTubeAPI:
    ...
    async def video(self, link: str, videoid: Union[bool, str] = None):
        ...
        if "&" in link: link = link.split("&")[0]
        try:
            downloaded_file = await ytdl_fallback_download(link, "video")
            if downloaded_file:
    ...
    async def download(
        ...
        try:
            ...
            if video: downloaded_file = await ytdl_fallback_download(link, "video", title=file_title)
