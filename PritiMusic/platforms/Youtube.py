...
AVAILABLE_APIS = [
    ("ShrutiBots", API_URL, API_KEY),
    ("Xbit", YTPROXY_URL, YT_API_KEY),
    ("Worker", WORKER_FALLBACK_API_URL, WORKER_FALLBACK_API_KEY),
    ("Inflex", INFLEX_API_URL, INFLEX_API_KEY)
]
...
# 🚀 1. FAST DOWNLOAD VIA MULTIPLE APIs (THE RACE)
async def fetch_from_single_api(api_name, api_url, api_key, video_id, download_type, final_file_path):
    temp_file_path = f"{final_file_path}.{api_name}.tmp"
    ...
    try:
        ...
        if os.path.exists(temp_file_path):
            os.remove(temp_file_path)
        raise ValueError(f"{api_name} failed: {e}")
...
async def api_download(video_id: str, download_type: str, title: str = None) -> str:
    ...
    tasks = set()
    for name, url, key in AVAILABLE_APIS:
        if url and key:
            task = asyncio.create_task(
                fetch_from_single_api(name, url, key, video_id, download_type, file_path), 
                name=name
            )
            tasks.add(task)
    ...
    while tasks:
        ...
        for task in done:
            try:
                winner_path = task.result()
                if winner_path:
                    LOGGER.info(f"⚡ THE SHIV's API Engine: {task.get_name()} won the race for '{title or video_id}'!")
                    break
            except Exception as e:
                LOGGER.debug(f"API Miss: {e}")
